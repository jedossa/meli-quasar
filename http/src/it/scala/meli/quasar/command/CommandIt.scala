...
import meli.quasar.infraestructure.config.AppConfig
import meli.quasar.infraestructure.repository.rows.AccountRow
import meli.quasar.program.modules
...
import meli.quasar.repository.rows.{SatelliteRow, TransactionRow}
import meli.quasar.dtos.{TopSecretDTO, TransactionDto}
import nu.authorizer.domain._
import nu.authorizer.infraestructure.repository.rows.AccountRow
import nu.authorizer.program.modules.LiveValidator
import nu.authorizer.serde.AccountDto
...
class CommandIt extends AnyFunSuite with ScalaCheckDrivenPropertyChecks with CoreItGenerator {
  ...
  implicit val accountDtoDecoder: Decoder[TopSecretDTO] = { json =>
  ...
  test("Should get the account's current state") {
    val accountStorage: Option[SatelliteRow] = none[SatelliteRow]
    ...
    forAll(accountWithActiveCard, Gen.listOf(regularTransaction)) { (account, transactions) =>
      ...
      val program = for {
        ...
        xa        <- Ref.of[IO, Option[SatelliteRow]](accountStorage).map(InMemoryTransactor(_, accountStorage))
        xt <- Ref
        ...
        .flatMap(_._1.lastOption.flatMap(decode[TopSecretDTO](_).toOption))
    ...
    forAll(accountWithActiveCard) { account =>
      val program = for {
        ...
        xt <- Ref
          .of[IO, TreeMap[TransactionRow.Key, TransactionRow]](transactionStorage)
          .map(InMemoryTransactor(_, transactionStorage))
        repository = modules.InMemoryRepository[IO](xa, xt)
        validator = LiveValidator[IO](repository)
        service = modules.LiveService[IO](repository, validator, config)
        inputs <- TestConsole.inputs
